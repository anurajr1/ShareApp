package com.anu.developers3k.shareapp.bottomNavigation;
...
import com.anu.developers3k.shareapp.bottomNavigation.notification.Notification;
import com.anu.developers3k.shareapp.bottomNavigation.notification.NotificationHelper;
...
public class BottomNavigation extends FrameLayout {
	...
	private void createClassicItems(LinearLayout linearLayout) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (itemsEnabledStates[i]) {
				...
				iconDrawable = forceTint ? BottomHelper.getTintDrawable(items.get(i).getDrawable(context),
	...
	private void updateItems(final int itemIndex, boolean useCallback) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (i == itemIndex) {
				...
				icon.setSelected(true);
				BottomHelper.updateTopMargin(icon, inactiveMarginTop, activeMarginTop);
				BottomHelper.updateLeftMargin(notification, notificationInactiveMarginLeft, notificationActiveMarginLeft);
				BottomHelper.updateTextColor(title, itemInactiveColor, itemActiveColor);
				BottomHelper.updateTextSize(title, inactiveSize, activeSize);
				if (forceTint) {
					BottomHelper.updateDrawableColor(context, items.get(itemIndex).getDrawable(context), icon,
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && colored) {
					...
					BottomHelper.updateViewBackgroundColor(this, currentColor,
				...
				icon.setSelected(false);
				BottomHelper.updateTopMargin(icon, activeMarginTop, inactiveMarginTop);
				BottomHelper.updateLeftMargin(notification, notificationActiveMarginLeft, notificationInactiveMarginLeft);
				BottomHelper.updateTextColor(title, itemActiveColor, itemInactiveColor);
				BottomHelper.updateTextSize(title, activeSize, inactiveSize);
				if (forceTint) {
					BottomHelper.updateDrawableColor(context, items.get(currentItem).getDrawable(context), icon,
	...
	private void updateSmallItems(final int itemIndex, boolean useCallback) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (i == itemIndex) {
				...
				if (titleState != TitleState.ALWAYS_HIDE) {
					BottomHelper.updateTopMargin(icon, inactiveMargin, activeMarginTop);
					BottomHelper.updateLeftMargin(notification, notificationInactiveMarginLeft, notificationActiveMarginLeft);
					BottomHelper.updateTopMargin(notification, notificationInactiveMarginTop, notificationActiveMarginTop);
					BottomHelper.updateTextColor(title, itemInactiveColor, itemActiveColor);
					BottomHelper.updateWidth(container, notSelectedItemWidth, selectedItemWidth);
				}

				BottomHelper.updateAlpha(title, 0, 1);
				if (forceTint) {
					BottomHelper.updateDrawableColor(context, items.get(itemIndex).getDrawable(context), icon,
							itemInactiveColor, itemActiveColor, forceTint);
				}
				...
				if (titleState != TitleState.ALWAYS_HIDE) {
					BottomHelper.updateTopMargin(icon, activeMarginTop, inactiveMargin);
					BottomHelper.updateLeftMargin(notification, notificationActiveMarginLeft, notificationInactiveMarginLeft);
					BottomHelper.updateTopMargin(notification, notificationActiveMarginTop, notificationInactiveMarginTop);
					BottomHelper.updateTextColor(title, itemActiveColor, itemInactiveColor);
					BottomHelper.updateWidth(container, selectedItemWidth, notSelectedItemWidth);
				}

				BottomHelper.updateAlpha(title, 1, 0);
				if (forceTint) {
					BottomHelper.updateDrawableColor(context, items.get(currentItem).getDrawable(context), icon,
							itemActiveColor, itemInactiveColor, forceTint);
				}
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (updateStyle) {
				...
				if (notificationBackgroundDrawable != null) {
					...
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
						notification.setBackground(BottomHelper.getTintDrawable(defautlDrawable,
								currentBackgroundColor, forceTint));
					} else {
						notification.setBackgroundDrawable(BottomHelper.getTintDrawable(defautlDrawable,
								currentBackgroundColor, forceTint));
					}
				}
			}
	 ...
	 */
	public void addItemAtIndex(int index, BottomNavigationItem item) {
